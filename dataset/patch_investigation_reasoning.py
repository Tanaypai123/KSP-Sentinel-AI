with open('/Users/tanaysharma/Desktop/KSP-Sentinel-AI/dataset/app/ai/investigation_reasoning_engine.py', 'r') as f:
    content = f.read()

import re

# Update InvestigationConclusion dataclass
new_conclusion = """@dataclass
...
content = re.sub(r"@dataclass\nclass InvestigationConclusion:.*?    supporting_evidence: List\[str\]", new_conclusion, content, flags=re.DOTALL)

# Now rewrite the entire InvestigationReasoningEngineStage.run()
...
content = re.sub(r"    @staticmethod\n    def run\(context: Any\) -> Any:.*        return context", new_run, content, flags=re.DOTALL)

with open('/Users/tanaysharma/Desktop/KSP-Sentinel-AI/dataset/app/ai/investigation_reasoning_engine.py', 'w') as f:
    f.write(content)
